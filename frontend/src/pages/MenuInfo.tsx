...
export default function MenuInfo({
  navigation,
  route,
}: MenuInfoProps): React.JSX.Element {
  ...
  return (
    <SafeAreaView style={styles.container}>
        ...
        <View
          ...
          {['순한맛', '중간맛', '매운맛'].map(flavor => (
            <View key={flavor} style={styles.flavoursBox}>
              ...
              <Text>
                {menu ? `${formatPrice(menu?.price)}원` : '가격 정보 없음'}
              </Text>
            </View>
          ))}
        </View>
          ...
          <View style={styles.flavoursBox}>
            ...
            {options.length > 1 && options[1].option ? (
              <View style={styles.detailRound}>
                <Text>
                  {options[1].option.is_required === 'required'
                    ? '필수'
                    : '선택'}
                </Text>
              </View>
          ...
          options[1].details.length > 0 ? (
            options[1].details.map(detail => (
              <View key={detail.title} style={styles.flavoursBox}>
                ...
                <Text style={styles.flavoursPrice}>{`${formatPrice(
                  detail.price,
                )}원`}</Text>
              </View>
            ))
      ...
      <BottomButton
        ...
        onPress={handleOrder}
        checked={count > 0 && selectedFlavor != null}
        color="#EC424C"
      />
    </SafeAreaView>
  );
